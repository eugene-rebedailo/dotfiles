...
function M.build_dll_path(project_root)
...
function M.get_launch_profiles_with_env(project_root)
    local launch_settings_path = vim.fn.findfile('launchSettings.json', project_root .. '/Properties/')
    if launch_settings_path == '' then
        return {}
    end

    local launch_settings_file = assert(io.open(launch_settings_path --[[@as string]], 'rb'))
    local settings_content = launch_settings_file:read '*a'
    launch_settings_file:close()
    local settings_json = vim.json.decode(settings_content)
    local launch_profiles = {}
    for key, _ in pairs(settings_json.profiles) do
        table.insert(launch_profiles, {
            name = key,
            env = settings_json.profiles[key].environmentVariables,
        })
    end
    return launch_profiles
end

function M.discover_project_root()
